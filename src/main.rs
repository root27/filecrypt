        ...
        if cmd.args_present() {
            ...
            if let Some(path) = cmd.get_one::<String>("file") {
                ...
                if cmd.get_flag("encrypt") {
                    ...
                    print!("Would you like to save the content to the same file ? (y/n): ");

                    std::io::stdout().flush().unwrap();

                    let mut user_choice  = String::new();

                    std::io::stdin().read_line(&mut user_choice).expect("Failed to read line");
                    ...
                    print!("Please provide the file name to save encryted content: ");
                    ...
                    fs::write(file_name.trim(), token).expect("could not write file");
